...
def _prompt_manual_decks() -> np.ndarray:
    try:
        raw_count = input("How many additional decks would you like to generate? (0 to skip): ").strip()
    except EOFError:
    ...
    try:
        ...
        print("[manual] Invalid number entered; skipping deck generation.")
        return np.empty((0, 52), dtype=np.uint8)
    ...
    if raw_seed:
        try:
            ...
            print("[manual] Seed must be an integer. Using random seed instead.")
    ...
    print(f"[manual] Generated {stacked.shape[0]} deck(s) using seed {seed}.")
    return stacked
...
def _score_generated_decks(counts: dict[str, np.ndarray], current_total: int) -> tuple[int, int]:
    if N_DECKS <= current_total:
        print(f"[auto] Target deck count already satisfied (total={current_total}).")
        return 0, current_total

    decks_needed = N_DECKS - current_total
    print(f"[auto] Generating {decks_needed} additional deck(s) to reach {N_DECKS}.")
    ...
    while produced < decks_needed:
        ...
        produced += batch_size
        batch_counter += 1
        print(f"[auto] Scored batch {batch_idx} ({batch_size} decks)")
...
def _build_heatmaps(total_decks: int, counts: dict[str, np.ndarray]) -> None:
    if total_decks == 0:
        print("[viz] No decks scored; skipping heatmaps.")
        return
    ...
    tricks_fig = save_p2_win_prob_heatmap_from_counts(
        ...
        total_decks,
        out_dir=str(FIG_DIR),
        filename="p2_win_tricks.png",
        title=f"Humble–Nishiyama P2 Win Probabilities (tricks, n={total_decks})",
    )
    cards_fig = save_p2_win_prob_heatmap_from_counts(
        ...
        total_decks,
        out_dir=str(FIG_DIR),
        filename="p2_win_cards.png",
        title=f"Humble–Nishiyama P2 Win Probabilities (cards, n={total_decks})",
    )
    print("[viz] Tricks heatmap:", tricks_fig)
    print("[viz] Cards heatmap:", cards_fig)
...
def main() -> None:
    ...
    print(f"[summary] Auto decks added: {new_auto}, manual decks: {total_manual}, total: {total_decks}")
